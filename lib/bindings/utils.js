var _previousSelectedNode = null; 
function neighbourhoodHighlight(params) {
  var _wasHighlightActive = highlightActive;
  var _wasSelectedNode = _previousSelectedNode;
  ...
  if (params.nodes.length > 0) {
    ...
    for (let nodeId in allNodes) {
      ...
      if (window._intervalHighlightedIds && window._intervalHighlightedIds.indexOf(nodeId) !== -1) continue;
    ...
    for (i = 0; i < connectedNodes.length; i++) {
      if (allNodes[connectedNodes[i]]) {
        ...
        (window._intervalHighlightedIds && window._intervalHighlightedIds.indexOf(connectedNodes[i]) !== -1)
        ? '#ff6b6b'
    ...
    (window._intervalHighlightedIds && window._intervalHighlightedIds.indexOf(selectedNode) !== -1)
    ? '#ff6b6b'  // keep interval highlight colour
    : (allNodes[selectedNode].lightColour || nodeColors[selectedNode]);
  ...
  if (params.nodes.length > 0) {
    ...
    // Only zoom if this is a fresh selection (not switching between already-highlighted nodes)
    if (!_wasHighlightActive || _wasSelectedNode !== selectedNode) {
      network.fit({
...
function highlightFilter(filter) {
  ...
  var propAlias = { compartment: 'comp1' };
  var selectedProp = propAlias[filter.property] || filter.property;
  ...
  for (var nodeId in allNodes) {
    ...
    var threshold = extractMb(filterValues[0]);
    var numVal = extractMb(rawVal);
